...
require_once 'H:\inetpub\lib\sqlsrvLibFL.php';												// Nov 22 2023
...
ini_set("error_log", "H:\inetpub\lib\FL_ESB\\".$mylocation->path."\\ReconMQ_WB_dual_Error.txt");
//	$handle242 = connectToDB(0);
	$handle242 = connectDB_FL();															// Nov 22, 2023
 ...
 */
	...
	for ($i = 0; $i < $numLoops; $i++){						// go forward by 1 day, repeat number specified times
		$tst = strcmp($MQday->format("Y-m-d"), $params['ignoreDate'] );
		if (strcmp($MQday->format("Y-m-d"), $params['ignoreDate'] )== 0){
			fwrite($fp, "\r\n ignoring date".$params['ignoreDate']);
			echo "<br> $i ignoring date".$params['ignoreDate'] ."<br>";
		}

		fwrite($fp, "\r\n \r\n \r\n for theDay ". $MQday->format("Y-m-d")."\r\n");
		echo "<br> 44 i is $i  day is ".$MQday->format("Y-m-d") ." ignoreDate is ".$params['ignoreDate'] ."<br>"; 
		var_dump($tst);
		...
		$WBrow242 = getFromWBdual( $MQday,  &$dBugArray, 0);			// get data from WB, from 242 if running in _dev_ or WB if running in _prod_ 
		$MQdayClone = clone $MQday;
		if (strcmp($MQday->format("Y-m-d"), $params['ignoreDate'] ) !== 0)
...
function getFromWBdual( $firstDay,  $dBugArray, $mode){
	...
	if (isset($params['PatID']))	
		{$tat = print_r($patArray[$params['PatID']], true); fwrite($tf, "\r\n from WB \r\n"); fwrite($fp, $tat);}		// write out the element being tested
	return $patArray;
}
function isIgnoreDate($MqApt_DtTm){
	echo "<br> MqApptDtTim is 888 <br>"; var_dump($MqApt_DtTm);
}
/**
...
function getFromMQ_dual($mqDay){
   ...
   $selStr = "SELECT TOP(100)  Sch_Id, App_DtTm, IDA, PAT_NAME, Duration_time, LOCATION, SysDefStatus, CGroup
    ...
    while ($assoc = $dB->getAssoc()){
		...
		echo "<br> 150 <br>";	echo "<pre>"; print_r($assoc); echo "</pre>";
       ...
       /*******  add a appt for testing *********/ 
		if (isset($params['PatID']) && strpos($assoc['IDA'], $params['PatID']) !== FALSE){					// this is the PatID to add a appt for 
	...
	if (isset($pRow) && is_array($pRow))											// end of main loop
...
function makeOnlyEditFile(){
	...
	$CWD = getcwd();
	if (strpos($CWD, '_qa_') !== FALSE) $loc = '_qa_';
	if (strpos($CWD, '_dev_') !== FALSE) $loc = '_dev_';
	if (strpos($CWD, '_prod_') !== FALSE) $loc = '_prod_';
    	$fileName = "H:\\inetpub\\esblogs\\".$loc."\\MQ-WB_OnlyEdits.log"; 
echo "<br> 424 8888  onlyEdits filename is $fileName <br>";	
		try {
			$fp = fopen($fileName, "a+");
			if ($fp) echo "\r\n $fileName opened";
		}
...
echo "<br> from onlyEdits <br>"; var_dump($fp);
